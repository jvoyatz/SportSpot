...
class SportCategoryEntityMapperTest: BaseTest(){

    @Test
    fun `convert entity to domain model`(){
        //given

        val sportEvents = data?.get(0)!!
        val entity = sportEvents.dtoToDomain().domainToEntity().copy(id = "kotlin")
        ...
        Truth.assertThat(domain.id).isEqualTo("kotlin")
    }

    @Test
    fun `convert domain to entity model`(){
        //given
        val event = data?.get(0)!!
